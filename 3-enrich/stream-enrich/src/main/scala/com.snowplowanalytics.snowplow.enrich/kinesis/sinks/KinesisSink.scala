...
class KinesisSink(provider: AWSCredentialsProvider,
  ...
  // I just made in anonymous placeholders
  val clientConfig = new ClientConfiguration()
  clientConfig.setProxyHost("{{proxy-url-placeholder}}")
  clientConfig.setProxyPort(8099)
  clientConfig.setProxyUsername("{{proxy-username-placeholder}}")
  clientConfig.setProxyPassword("{{proxy-password-placeholder}}")
  clientConfig.setPreemptiveBasicProxyAuth(false)
 val client = new AmazonKinesisClient(provider,clientConfig)
   ...
   */
  def streamExists(name: String, timeout: Int = 120): Boolean = {
   // println('name of stream '+name)
